				...
				while (y < totalChildren) {
					...
					if (_node.parent) {
						var _ring = {
							...
							x: _node.x,
							y: _node.y,
							r: _node.parent.ring / orbitDepthAdjust(_node) * (_currentRing / _rings)
						};
					} else {
						var _ring = {
							source: _node,
							x: _node.x,
							y: _node.y,
							r: (orbitSize[0] / 2) * (_currentRing / _rings)
						};
					}
					...
					var piedValues = thisPie(childrenAccessor(_node).filter(function(d, i) {
						return i >= y && i <= y + ringSize - 1
					}));
